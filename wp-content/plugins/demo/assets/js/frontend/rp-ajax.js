...
jQuery(document).ready(function($) {
  ...
  // Process checkout
  $(document).on('click', '#rpress_purchase_form  #rpress_next_button', function(e) {
      ...
      $.post(rpress_global_vars.ajaxurl, $('#rpress_purchase_form').serialize() + '&action=rpress_process_confirm&rpress_ajax=true', function(data) {
          try { 
                var data = JSON.parse( data );
            }
            catch(err) {  //We can also throw from try block and catch it here
                swal("ohhhh...!!", "Please Enter all requied data.!", "error");
            }
            finally {
        ...
        if ( $.trim(data.status) == 'success' ) {
          ...
          if(data.shipping == 'null'){
            swal("ohhhh...!!", "Sorry we are not delivered on this address.!", "error");
            $('.rpress-loading-ajax').remove();
            $('#rpress_next_button').val(complete_purchase_val);
            $('#rpress_next_button').prop('disabled', false);
              
          }else{
                var decoded = $("<div/>").html(data.symbol).text();
                swal({
                      html:true,
                      title: "Shipping Charge is "+decoded+data.shipping,
                      text: "Approx distance is "+data.dis.km,
                      icon: "warning",
                      buttons: true,
                      dangerMode: true,
                    })
  ...
  $(document).on('click', '#rpress_purchase_form #rpress_purchase_submit [type=submit]', function(e) {
      ...
      var rpressPurchaseform = document.getElementById('rpress_purchase_form');
        console.log(rpressPurchaseform);
      ...
      $.post(rpress_global_vars.ajaxurl, $('#rpress_purchase_form').serialize() + '&action=rpress_process_checkout&rpress_ajax=true', function(data) {
        ...
        if ( $.trim(data) == 'success' ) {
          ...
          if($('input[name=rpress-purchase-var]').val() == 'needs-to-register'){
            swal("Register Successfull..!", "Please login now.!", "success");
          }
...
$(document).on('click', '#rpress_purchase_form #rpress_purchase_submit #rpress-update-address', function(e) {
     ...
     var rpressPurchaseform = document.getElementById('rpress_purchase_form');
      ...
      $.ajax({
        type  : "POST",
        data  : $('#rpress_purchase_form').serialize()+ '&action=rpress_process_update_address_custome',
