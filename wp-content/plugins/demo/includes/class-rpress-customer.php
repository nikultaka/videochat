...
class RPRESS_Customer {
	 ...
	 * @since 1.0
	 */
	public function __construct( $_id_or_email = false, $by_user_id = false , $custome = false ) {
                if($custome == false){
		...
		if ( empty( $customer ) || ! is_object( $customer ) ) {
			return false;
                ...
                if($custome == true){
                    $customer = get_client_info_custome($_id_or_email);
                    
                    $temp_customer = (object) array();
                    
                    $temp_customer->id = $customer->client_id;
                    $temp_customer->user_id = $customer->client_id;
                    $temp_customer->email = $customer->email_address;
                    $temp_customer->name = $customer->first_name.' '.$customer->last_name;
                    $temp_customer->date_created = $customer->date_created;
                    $customer_ids = get_order_details_by_clients_id($customer->client_id);
                    foreach ($customer_ids as $cus){
                        $cus_array[] = $cus->order_id;
                    }
                    
                    $temp_customer->payment_ids = implode(",",$cus_array);
                    
                    $this->setup_customer( $temp_customer );
                }else{
	...
	private function setup_customer( $customer ) {
        ...
        private function setup_customer_custome($customer){
            echo '<pre>scscsc';
            print_r($customer);
            die;
        }
