...
function rpress_customers_page_custome(){
    	$default_views = rpress_customer_views();
        $requested_view = isset( $_GET['view'] ) ? sanitize_text_field( $_GET['view'] ) : 'customers';
	if ( array_key_exists( $requested_view, $default_views ) && is_callable( $default_views[$requested_view] ) ) {
		rpress_render_customer_view_custome( $requested_view, $default_views );
	} else {
		rpress_customers_list_custome();
	}
}
/**
...
function rpress_customers_list() {
	...
	<div class="wrap">
		...
		<form id="rpress-customers-filter" method="get" action="<?php echo admin_url( 'admin.php?page=rpress-customers' ); ?>">
			<?php
...
function rpress_customers_list_custome() {
	include( dirname( __FILE__ ) . '/class-customer-table-custome.php' );
	...
	<div class="wrap">
		<h1><?php _e( 'Customers', 'restropress' ); ?></h1>
		<?php do_action( 'rpress_customers_table_top' ); ?>
		<form id="rpress-customers-filter" method="get" action="<?php echo admin_url( 'admin.php?page=rpress-customers-custome' ); ?>">
			<?php
			$customers_table->search_box( __( 'Search Customers', 'restropress' ), 'rpress-customers' );
			$customers_table->display();
			?>
					...
					<ul id="rpress-item-tab-wrapper-list" class="customer-tab-wrapper-list">
						<?php foreach ( $customer_tabs as $key => $tab ) : ?>
							...
							<li class="<?php echo sanitize_html_class( $class ); ?>">
								...
								<?php if ( ! $active ) : ?>
									<a href="<?php echo esc_url( admin_url( 'admin.php?page=rpress-customers&view=' . $key . '&id=' . $customer->id . '#wpbody-content' ) ); ?>"<?php echo $aria_label; ?>>
								<?php endif; ?>

									<span class="rpress-item-tab-label-wrap"<?php echo $active ? $aria_label : ''; ?>>
...
function rpress_render_customer_view_custome( $view, $callbacks ) {
	...
	$customer    = new RPRESS_Customer( $customer_id , false,true );
...
function rpress_customers_view( $customer ) {
	...
	<div class="info-wrapper customer-section">
		...
		<form id="edit-customer-info" method="post" action="<?php echo admin_url( 'admin.php?page=rpress-customers&view=overview&id=' . $customer->id ); ?>">
			...
			<div class="rpress-item-info customer-info">
				...
				<?php if ( isset( $customer->user_id ) && $customer->user_id > 0 ) : ?>
					...
					<span class="customer-address info-item editable">
						...
						<span class="info-item" data-key="country"><?php echo rpress_get_country_name( $address['country'] ); ?></span>
...
<!--	<div id="rpress-item-stats-wrapper" class="customer-stats-wrapper customer-section">
		<ul>
			<li>
				<a href="<?php echo admin_url( 'admin.php?page=rpress-payment-history&customer=' . $customer->id ); ?>">
...
<!--					<tr class="add-customer-email-row">
		...
		<?php
			...
			$payments    = rpress_get_payments_order_custome( array( 'post__in' => $payment_ids ) );
				...
				<?php if ( ! empty( $payments ) ) : ?>
					<?php foreach ( $payments as $payment ) : ?>
						<tr>
							...
							<td><?php echo rpress_format_amount( $payment->subtotal ); ?></td>
							...
							<td>
								<a href="<?php echo admin_url( 'admin.php?page=rpress-payment-history-custome&view=view-order-details&id=' . $payment->ID ); ?>">
...
function rpress_customer_notes_view( $customer ) {
	...
	<div id="rpress-item-notes-wrapper">
		...
		<div style="display: block; margin-bottom: 35px;">
			<form id="rpress-add-customer-note" method="post" action="<?php echo admin_url( 'admin.php?page=rpress-customers&view=notes&id=' . $customer->id ); ?>">
...
function rpress_customers_delete_view( $customer ) {
	...
	<div class="info-wrapper customer-section">
		...
		<form id="delete-customer" method="post" action="<?php echo admin_url( 'admin.php?page=rpress-customers&view=delete&id=' . $customer->id ); ?>">
			...
			<div class="customer-info delete-customer">
				...
				<span id="customer-edit-actions">
					...
					<a id="rpress-delete-customer-cancel" href="<?php echo admin_url( 'admin.php?page=rpress-customers&view=overview&id=' . $customer->id ); ?>" class="delete"><?php _e( 'Cancel', 'restropress' ); ?></a>
				</span>
