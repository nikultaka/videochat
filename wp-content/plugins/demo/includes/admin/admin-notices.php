...
function rpress_admin_messages() {
	...
	if ( isset( $_GET['page'] ) && 'rpress-payment-history' == $_GET['page'] && current_user_can( 'view_shop_reports' ) && rpress_is_test_mode() ) {
		...
		add_settings_error( 'rpress-notices', 'rpress-payment-sent', sprintf( __( 'Note: Test Mode is enabled, only test payments are shown below. <a href="%s">Settings</a>.', 'restropress' ), admin_url( 'admin.php?page=rpress-settings' ) ), 'updated' );
	}
	...
	if ( ( empty( $rpress_options['purchase_page'] ) || 'trash' == get_post_status( $rpress_options['purchase_page'] ) ) && current_user_can( 'edit_pages' ) && ! get_user_meta( get_current_user_id(), '_rpress_set_checkout_dismissed' ) ) {
		...
		/* translators: %s: admin settings link */
			echo '<p>' . sprintf( __( 'No checkout page has been configured. Visit <a href="%s">Settings</a> to set one.', 'restropress' ), admin_url( 'admin.php?page=rpress-settings' ) ) . '</p>';
