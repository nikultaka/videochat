...
class RP_FoodItem_Meta_Boxes{
  ...
  public static function save_meta_boxes( $post_id, $post ) {
    ...
    // Check the nonce.
    if ( empty( $_POST['restropress_meta_nonce'] ) || ! wp_verify_nonce( wp_unslash( $_POST['restropress_meta_nonce'] ), 'restropress_save_data' ) ) {
      return;
    }
    ...
    foreach( $fields as $field ) {
      ...
      if( $field != 'addons' && !empty( $_POST[$field]  ) ) {
        $value = apply_filters( 'rpress_metabox_save_' . $field, $_POST[$field] );
    ...
    if ( !empty( $_POST['addons'] ) && count ( $_POST['addons'] ) > 0 ) {
      ...
      update_post_meta( $post_id, '_addon_items', $addons );
    ...
    if ( isset( $_POST['addon_category'] ) &&
      !empty( $_POST['addon_category'] ) &&
      count( $_POST['addon_category'] ) > 0 ) {
