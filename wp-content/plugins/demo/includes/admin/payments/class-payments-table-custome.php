 ...
 * @since  1.0.0
 */
class RPRESS_Payment_History_Table_Custome extends WP_List_Table {
	...
	public $out_for_deliver_count;
        public $initial_order_count;
        public $inkitchen_count;
	...
	public function __construct() {
		...
		$this->base_url = admin_url( 'admin.php?page=rpress-payment-history-custome' );
	...
	public function service_type_filters() { ?>
    	...
    	<div class="rpress-service-type-filters-wrap">
      		<ul class="subsubsub">
	        	...
	        	<li>
	          		<a href="<?php echo admin_url('/admin.php?page=rpress-payment-history-custome'); ?>">
	          			<?php echo esc_html_e( 'All', 'restropress' ); ?>
	          		</a> |
	          	</li>
	          	...
	          	foreach( $get_service_types as $service_key => $service_label ) : ?>
	          		<li>
	          			<a href="<?php echo admin_url('/admin.php?page=rpress-payment-history-custome&service_type='.$service_key); ?>">
	...
	public function advanced_filters() {
		...
		<div id="rpress-payment-filters">
			...
			<span>/</span>
			<span>
				<label for="service-date-filter"><?php _e( 'Service Date:', 'restropress' ); ?></label>
				<input type="text" id="service-date-filter" name="service-date" class="rpress_datepicker" value="<?php echo $service_date; ?>" placeholder="mm/dd/yyyy"/>
			</span>
			...
			<?php if( ! empty( $service_date ) || ! empty( $start_date ) || ! empty( $end_date ) || 'all' !== $selected_gateway ) : ?>
				<a href="<?php echo admin_url( 'admin.php?page=rpress-payment-history-custome' ); ?>" class="button-secondary"><?php _e( 'Clear Filter', 'restropress' ); ?></a>
			<?php endif; ?>
    ...
    $order_preview = '<a href="#" class="order-preview" data-order-id="' . absint( $payment->ID ) . '" title="' . esc_attr( __( 'Preview', 'restropress' ) ) . '"><span>' . esc_html( __( 'Preview', 'restropress' ) ) . '</span></a>
      <a class="" href="' . add_query_arg( 'id', $payment->ID, admin_url( 'admin.php?page=rpress-payment-history-custome&view=view-order-details' ) ) . '">#' . $payment->ID . ' ' . $customer_name . '</a><span class="rp-service-type badge-' . $service_type . ' ">' . rpress_service_label( $service_type ) . '</span>';
	...
	public function column_customer( $payment ) {
		...
		if( ! empty( $customer_id ) ) {
			...
			$email = rpress_get_payment_user_email( $payment->ID );
			$value = '<a href="' . esc_url( admin_url( "admin.php?page=rpress-payment-history-custome&s=$email" ) ) . '">' . __( '(customer missing)', 'restropress' ) . '</a>';
		}
	...
	public function get_payment_counts() {
		...
		$payment_count          	= payment_count_details( $args );
                foreach ($payment_count as $count){
                    switch ( $count->status ) {
                                case 'accepted':
                                        $this->completed_count   	= (isset($count->count_status))? $count->count_status : 0;
                                        break;
                                case 'pending':
                                        $this->pending_count    	=  (isset($count->count_status)) ? $count->count_status : 0 ;
                                        break;
                                case 'paid':
                                        $this->paid_count 			=  (isset($count->count_status)) ? $count->count_status : 0 ;
                                break;
                                case 'inkitchen':
                                         $this->out_for_deliver_count 			=  (isset($count->count_status)) ? $count->count_status : 0 ;
                                        break;
                                case 'initial_order':
                                     $this->initial_order_count 			=  (isset($count->count_status)) ? $count->count_status : 0 ;
                                    break;
                                default:
		...
		$this->total_count = intval( $this->completed_count ) + intval( $this->pending_count ) + intval( $this->paid_count ) + intval( $this->out_for_deliver_count ) + intval( $this->initial_order_count );
		// foreach( $payment_count as $count ) {
	...
	public function payments_data() {
		...
		return $p_query->get_payments_custome();
          ...
          <section class="rp-backbone-modal-main" role="main">
            ...
            <footer>
              <div class="inner">
                ...
                <a class="button button-primary button-large" aria-label="<?php esc_attr_e( 'Edit this order', 'restropress' ); ?>" href="<?php echo esc_url( admin_url( 'admin.php?page=rpress-payment-history-custome&view=view-order-details' ) ); ?>&id={{ data.id }}"><?php esc_html_e( 'Edit', 'restropress' ); ?></a>
