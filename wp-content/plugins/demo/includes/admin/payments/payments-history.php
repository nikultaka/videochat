...
function rpress_payment_history_page_custome() {
	
	$rpress_payment = get_post_type_object( 'rpress_payment' );

	if ( isset( $_GET['view'] ) && 'view-order-details' == $_GET['view'] ) {
		require_once RP_PLUGIN_DIR . 'includes/admin/payments/view-order-details-custome.php';
	} else {
		require_once RP_PLUGIN_DIR . 'includes/admin/payments/class-payments-table-custome.php';
		$payments_table = new RPRESS_Payment_History_Table_Custome();
		$payments_table->prepare_items();
	?>
	<div class="wrap">
		<h1><?php echo $rpress_payment->labels->menu_name ?></h1>
		<?php do_action( 'rpress_payments_page_top' ); ?>
		<form id="rpress-payments-filter" method="get" action="<?php echo admin_url( 'admin.php?page=rpress-payment-history-custome' ); ?>">

			<input type="hidden" name="page" value="rpress-payment-history" />
			
			<?php $payments_table->views_custome() ?>
