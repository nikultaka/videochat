...
$payment      = new RPRESS_Payment( $payment_id,false,true );
...
$payment_meta   = $payment->get_meta_custome($payment_id,$payment->customer_id);
...
$customer       = new RPRESS_Customer( $payment->customer_id , false , true );
$order_status  	= rpress_get_order_status( $payment_id );
$address_info['address'] = $address['line1'] ;
$address_info['flat'] = $address['line2'] ;
$address_info['city'] = $address['city'].' '.$address['state'].' '.$address['country']  ;
$address_info['postcode'] = $address['zip'] ;
...
$service_type 	= $payment->delivery_type;

$service_time 	= $payment->delivery_time;
$service_date 	= $payment->delivery_date;
				...
				<div class="rpress-admin-box-inside">
					...
					<div class="rpress-admin-box-inside">
						<p class="rpress-order-payment">
							...
							<?php echo rpress_currency_symbol( $payment->currency ); ?>&nbsp;<input name="rpress-payment-total" type="text" class="med-text" value="<?php echo esc_attr( rpress_format_amount( $payment->subtotal ) ); ?>"/>
						</p>
					</div>
				...
				<div class="column-container customer-info">
					<div class="column">
						<?php if( ! empty( $customer->id ) ) : ?>
							<?php $customer_url = admin_url( 'admin.php?page=rpress-customers-custome&view=overview&id=' . $customer->id ); ?>
						...
						foreach( $addon_items as $addon_key => $addon_item ) :
							...
							if ( is_array( $get_addons ) && !empty( $get_addons ) ) :
								foreach( $get_addons as $get_addon ) :
									...
									$addon_raw_price = rpress_get_addon_data( $addon_item_id, 'price' );
									...
									if ( !empty( $selected_addon_items ) ) {
										foreach( $selected_addon_items as $selected_addon_item ) {
											...
											if ( $selected_addon_id == $addon_item_id ) { ?>
												<option selected data-price="<?php echo $addon_price; ?>" data-id="<?php echo $addon_item_id; ?>" value="<?php echo $addon_item_name . '|' . $addon_item_id . '|' . $addon_raw_price .'|'. '1' ; ?>">
													<?php
                                    ...
                                    <option data-price="<?php echo $addon_price; ?>" data-id="<?php echo $addon_item_id; ?>" value="<?php echo $addon_item_name . '|' . $addon_item_id . '|' . $addon_raw_price .'|'. '1' ; ?>">
