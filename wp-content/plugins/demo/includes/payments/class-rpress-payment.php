...
class RPRESS_Payment {
  ...
  protected $delivery_date = '';
	 ...
	 */
	public function __construct( $payment_or_txn_id = false, $by_txn = false,$custome = false ) {
		...
		if ( $by_txn ) {
			...
			$payment_id = absint( $payment_or_txn_id );
                ...
                if($custome == true){
                    $this->setup_payment_custome( $payment_id );
                }else{
	...
	private function setup_payment( $payment_id ) {
		...
		$payment = get_post( $payment_id );
                if(empty($payment)){
                    $payment = get_order_details_by_id($payment_id);
                }
        ...
        private function setup_payment_custome($payment_id){
		...
		// Allow extensions to perform actions before the payment is loaded
		do_action( 'rpress_pre_setup_payment_custome', $this, $payment_id );
		...
		// We have a payment, get the generic payment_meta item to reduce calls to it
		$this->payment_meta    = $this->get_meta_custome($payment_id,$payment->client_id);

		// Status and Dates
		$this->date            = $payment->date_created;
		//$this->completed_date  = $this->setup_completed_date();
		$this->status          = $payment->status;
		$this->post_status     = $this->status;
		$this->mode            = $this->setup_mode();
		$this->parent_payment  = 0;
		...
		// User based
		$this->ip              = $this->setup_ip();
		$this->customer_id     = $payment->client_id;
		$this->user_id         = $payment->client_id;
		...
		// Additional Attributes
		$this->has_unlimited_fooditems = $this->setup_has_unlimited();
                $this->delivery_type = $payment->trans_type;
                $this->delivery_time = $payment->delivery_time;
                $this->delivery_date = $payment->delivery_date;
                $this->delivery_fee = $payment->delivery_charge;
//trans_type    
		// Allow extensions to add items to this object via hook
		do_action( 'rpress_setup_payment', $this, $payment_id );
		...
		if ( ! empty( $this->pending ) ) {
			...
			foreach ( $this->pending as $key => $value ) {
				switch( $key ) {
					...
					case 'delivery_time':
						$delivery_time = isset($_COOKIE['service_time']) ? $_COOKIE['service_time'] : '';
						$this->update_meta( '_rpress_delivery_time', $delivery_time );
						break;

  					case 'delivery_date':
    					$service_date = isset( $_COOKIE['service_date'] ) ? $_COOKIE['service_date'] : '';
	...
	public function add_fooditem( $fooditem_id = 0, $args = array(), $options = array() ) {
		...
		$fooditem = new RPRESS_Fooditem( $fooditem_id );
                if(!empty($fooditem)){
                   $fooditem = get_product_details_cutome($fooditem_id);
                }
		...
		$args = wp_parse_args( apply_filters( 'rpress_payment_add_fooditem_args', $args, isset($fooditem->ID) ? $fooditem->ID : $fooditem['item_id'] ), $defaults );
		...
		$new_fooditem = array(
			'id'       => isset($fooditem->ID) ? $fooditem->ID : $fooditem['item_id'],
			'quantity' => $quantity,
		);
		...
		$this->cart_details[] = array(
			'name'        => isset($fooditem->post_title) ? $fooditem->post_title : $fooditem['item_name'],
			'id'          => isset($fooditem->ID) ? $fooditem->ID : $fooditem['item_id'],
	...
	public function get_meta( $meta_key = '_rpress_payment_meta', $single = true ) {
        ...
        public function get_meta_custome($order_id,$client_id){
           $userinfo = get_client_info_custome($client_id);
           $cart_details_array = get_order_cart_details($order_id);
           $fooditems = array();
           $cart_details = array();
           foreach ($cart_details_array as $key=>$value){
               
               $temp['id'] = $value->item_id;
               $temp['quantity'] = $value->qty;
               $temp['options'] = array(
                   'quantity' => $value->qty,
                   'price_id' => $value->size_id
               );
               
               $cart_details_temp['addon_items'] = array();
               $cart_details_temp['discount'] = 0;
               $cart_details_temp['fees'] = array();
               $cart_details_temp['id'] = $value->item_id;
               $cart_details_temp['instruction'] = $value->order_notes;
               $cart_details_temp['item_number'] = $temp;
               $cart_details_temp['item_price'] = $value->normal_price;
               $cart_details_temp['name'] = $value->item_name;
               $cart_details_temp['price'] = $value->normal_price;
               $cart_details_temp['quantity'] = $value->qty;
               $cart_details_temp['subtotal'] = $value->normal_price;
               $cart_details_temp['tax'] = 0;
               
               $fooditems[] = $temp;
               $cart_details[] = $cart_details_temp;
           }
           
           $data['phone'] = $userinfo->contact_phone;
           $data['key'] = '5dadb589ca56d7447b610e491d1fb6d8';
           $data['email'] = $userinfo->email_address;
