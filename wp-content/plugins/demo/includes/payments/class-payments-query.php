...
class RPRESS_Payments_Query extends RPRESS_Stats {
	...
	public function get_payments() {
        ...
        public function get_payments_custome() {
			...
			return $query->posts;
                ...
                if(!empty($get_order_details)){
                    ...
                    foreach ($get_order_details as $order){
                        $payment    = rpress_get_payment_custome( $order->order_id );
                        $this->payments[] = apply_filters( 'rpress_payment', $payment, $order->order_id, $this );
//                        $data = (object)[];   
//                        $data->ID = $order->order_id;
//                        $data->_ID = $order->order_id;
//                        $data->new = '';
//                        $data->number = $order->order_id;
//                        $data->mode = 'test';
//                        $data->total = $order->sub_total;
//                        $data->subtotal = $order->sub_total;
//                        $data->tax = $order->tax;
//                        $data->discounted_amount = $order->discounted_amount;
//                        $data->tax_rate = $order->tax;
//                        $data->fees = array();
//                        $data->fees_total = $order->card_fee;
//                        $data->discounts = 'none';
//                        $data->date = $order->delivery_date;
//                        $data->status = $order->status;
//                        $data->customer_id = $order->client_id;
...
//		}
        ...
        public function get_payments_by_order() {
		...
		do_action( 'rpress_pre_get_payments', $this );
                ...
                // $get_order_details = get_order_details_by_in_id($this->args);
                if(count($this->args['post__in']) > 0 ){
                    foreach ($this->args['post__in'] as $order){
                        $payment    = rpress_get_payment_custome( $order );
                        $this->payments[] = apply_filters( 'rpress_payment', $payment, $order->order_id, $this );
                    }
                }
