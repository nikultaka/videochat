...
class RPRESS_Cart {
	...
	public function __construct() {
		$action_array = array('rpress_show_delivery_options','rpress_show_products_cutome','rpress_check_service_slot_custome','rpress_add_to_cart_cutome'); //,'rpress_load_gateway'
		if(isset($_POST['action']) && in_array($_POST['action'],$action_array)) {
			
		} else {
			$current_url= basename("http://".$_SERVER['HTTP_HOST'].$_SERVER['REQUEST_URI']);
...
//                die;
        ...
        public function rpress_update_to_cart($cart_key,$fooditem_id, $options = array()){
        
            if($cart_key != ''){
                $this->contents[$cart_key] = $options;
	...
	public function get_item_discount_amount( $item = array(), $discount = false ) {
		...
		/* Added by Dipak to apply discounts on addon items */
		if( isset($item['addon_items']) && !empty($item['addon_items']) ) {
    		$addon_prices = wp_list_pluck($item['addon_items'], 'price');
    		$addon_prices = array_sum($addon_prices);
		...
		if ( ! $addon_price || false === $price ) {
	    	...
	    	//Check if Product is variable
	      	if ( $price_id > 0 ) {
	        	...
	        	$price = get_price_option_amount_size_id( $fooditem_id, $price_id );
	...
	public function get_discounted_amount( $discounts = false ) {
		...
		$items  = $this->get_contents_details();
                ...
                foreach ($items as $item){
                    $total_ammount += $item['price'];
                }
                if($discounts){
                  $discounts_number =  $discounts[0]; 
                }
                $discount_details  = get_discount_by_code( $discounts_number );
                
                if(!empty($discount_details)){
                  $amount = get_discount_value( $discount_details, $total_ammount ,true );
...
//		}
        ...
        public function get_offer_amount(){
            	$amount = 0.00;
		$items  = $this->get_contents_details();
                $total_ammount = 0;
		...
		$offer = get_offer_details_by_data($total_ammount);
                if(isset($offer->offer_percentage)){
                    $amount = ($total_ammount * $offer->offer_percentage) / 100;
                }
		return $amount;
        }
        public function get_offer_other_details(){
            	$amount = 0.00;
		$items  = $this->get_contents_details();
                $total_ammount = 0;
	...
	public function get_total( $discounts = false) {
		...
		$discounts    = (float) $this->get_discounted_amount($discounts);
                ...
                $offer    = (float) $this->get_offer_amount();
		$fees         = (float) $this->get_total_fees();
		$cart_tax     = (float) $this->get_tax();
                if( rpress_selected_service() == 'delivery' ) {
                    $shipping     = (float) rpress_get_shipping();
                }
                
		$total_wo_tax = $subtotal - ($discounts + $offer) + $fees;
		$total        = $subtotal - ($discounts + $offer) + $cart_tax + $fees + $shipping ;
	...
	public function get_item_name( $item = array() ) {
            ...
            $product_details = get_product_details_cutome( $item['id'] );
                    if(!empty($product_details)){
                       $item_title =  $product_details['item_name'];
                    }
                    if(empty( $item_title )){
