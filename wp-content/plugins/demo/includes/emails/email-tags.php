  ...
  <table id="email-table" class="display responsive no-wrap order-column" width="100%" style="border: 1px solid black; border-collapse: collapse;">
    <thead>
      <tr>
        <th style="border: 1px solid black; border-collapse: collapse;">
          <?php echo __( 'Food Item Purchased', 'restropress' ); ?>
        </th>
        <th class="center" style="text-align: center; border: 1px solid black; border-collapse: collapse;">
          <?php echo __( 'Quantity', 'restropress' ); ?>
        </th>
        <th class="center" style="text-align: center; border: 1px solid black; border-collapse: collapse;">
          <?php echo __( 'Price', 'restropress' ); ?>
        </th>
      </tr>
    </thead>
    ...
    <tbody>
      ...
      foreach ( $cart_items as $item ) :
        ...
        $price_id = rpress_get_cart_item_price_id( $item );
         ...
         if ( $show_names ) {
          ...
          if ( rpress_has_variable_prices( $fooditem_id ) ) {
            $price_id = isset( $item['item_number']['options']['price_id'] ) ? $item['item_number']['options']['price_id'] : '';
        ...
        $item_price = isset( $item['subtotal'] ) ? $item['subtotal']  : '0.00' ;
    ...
    <tfoot>
      <tr>
        <td colspan="2" class="right" style="border: 1px solid black; border-collapse: collapse; text-align:right; padding-right: 10px;">
          <strong><?php echo __( 'Sub Total', 'restropress' ); ?>:</strong>
          <br><br>

          <?php
          if ( ( $fees = rpress_get_payment_fees( $payment_id, 'fee' ) ) ) :
          ...
          <?php if( isset( $user['discount'] ) && $user['discount'] != 'none' ) : ?>
            <strong><?php echo __( 'Discount(s)', 'restropress' ); ?>:</strong>
            <br><br>
          <?php endif; ?>


          <strong><?php echo __( 'Total', 'restropress' ); ?>:</strong>

        </td>

        <td class="right" style="border: 1px solid black; border-collapse: collapse; text-align:center;">
          <span><?php echo rpress_currency_filter( rpress_format_amount( $cart_sub_total ) ); ?></span>
          <br><br>
