...
function rpress_process_purchase_form() {
	...
	if(isset($_POST['rpress_user_login'])) {
		if(signup_user_yii($_POST)) {

		} else {
			$valid_data = false;
			rpress_set_error( '', __( 'This email already exist', 'restropress' ) );
		}	
	}
	...
	$purchase_data = array(
		...
		'discount'     => rpress_get_cart_discounted_amount(), // Discounted amount
                ...
                'offer_data'  => RPRESS()->cart->get_offer_other_details(),
		...
		'distance' =>   rpress_get_shipping(true),
...
function rpress_process_confirm() {
	...
	if ( false === $valid_data || rpress_get_errors() || ! $user ) {
		...
		wp_update_user( array(
			'ID'         => get_current_user_id(),
			'first_name' => $_POST['rpress_first'],
			'last_name'  => $_POST['rpress_last']
		) );
	

	// Update the customer's address if different to what's in the database
	
        $user_details['delivery_address'] = array();
        $user_details['delivery_address']['address']   	= ! empty( $_POST['rpress_street_address'] ) ? sanitize_text_field( $_POST['rpress_street_address'] ) : '';
        $user_details['delivery_address']['flat']   		= ! empty( $_POST['rpress_apt_suite'] ) ? sanitize_text_field( $_POST['rpress_apt_suite'] ) : '';
        $user_details['delivery_address']['city']    		= ! empty( $_POST['rpress_city'] ) ? sanitize_text_field( $_POST['rpress_city'] ) : '';
        $user_details['delivery_address']['postcode']   = ! empty( $_POST['rpress_postcode'] ) ? sanitize_text_field( $_POST['rpress_postcode'] ) : '';
    
    
        update_user_meta( get_current_user_id(), '_rpress_user_delivery_address', $user_details['delivery_address'] ,false );
    
        $currency = rpress_get_currency();
        $data['currency'] = rpress_get_currency();
        $data['symbol'] = rpress_currency_symbol( $currency );
        $data['status'] = "success";
        $data['url'] = rpress_get_checkout_uri( 'confirm=1' );
        $data['shipping'] = rpress_format_amount( rpress_get_shipping() );
        $data['dis'] = rpress_get_shipping(true);
        echo json_encode($data);
	rpress_die();
}
add_action( 'rpress_process_confirm', 'rpress_process_confirm' );
add_action( 'wp_ajax_rpress_process_confirm', 'rpress_process_confirm' );
add_action( 'wp_ajax_nopriv_rpress_process_confirm', 'rpress_process_confirm' );
...
function rpress_purchase_form_validate_fields() {
	...
	if ( is_user_logged_in() ) {
		...
		if($user_role == 'administrator') {
			rpress_set_error( 'invalid_phone', __( 'Admin can not place the order', 'restropress' ) );	
		} 
...
function rpress_purchase_form_validate_discounts() {
	...
	if ( ! empty( $_POST['rpress-discount'] ) && __( 'Enter voucher code', 'restropress' ) != $_POST['rpress-discount'] ) {
...
function rpress_purchase_form_required_fields() {
		...
		if(isset($_POST['rpress_user_login'])) {
			$required_fields = array(
				...
				'rpress_user_login' => array(
					'error_id' => 'invalid_user_login',
					'error_message' =>  __('Please enter username', 'restropress')
				),
				'rpress_user_pass' => array(
					'error_id' => 'invalid_user_pass',
					'error_message' =>  __('Please enter password', 'restropress')
				)
			);
		} else {
			$required_fields = array(
        ...
        if(isset($_POST['rpress-purchase-var']) && $_POST['rpress-purchase-var'] != 'needs-to-register'){
...
function rpress_purchase_form_validate_user_login() {
	...
	if(login_user_yii($login_or_email,$user_pass)) {
		$client_data = $_SESSION['user_client_data'];
		$user_data = get_user_by( 'email', $login_or_email );
		if($user_data == '') {
			wp_create_user ($client_data->email_address, $_POST["rpress_user_pass"],$client_data->email_address);
		} else {
			
		}
		/*$valid_user_data = array(
			'user_id' => $client_data->client_id,
			'user_login' => $client_data->email_address,
			'user_email' => $client_data->email_address,
			'user_first' => $client_data->first_name,
			'user_last' => $client_data->last_name,
			'user_pass' => $client_data->password,
		);
		return $valid_user_data;*/
	} else {
		rpress_set_error( '', __( 'Email or password not found', 'restropress' ) );
		return $valid_user_data;
	}
...
function rpress_get_purchase_form_user( $valid_data = array() ) {
	...
	if ( $is_ajax ) {
		...
		if ( $valid_data['need_new_user'] === true ) {
			...
			/*if ( empty( $user ) || $user['user_id'] == -1 ) {
				rpress_set_error( 'invalid_user', __( 'This user is already exist', 'restropress' ) );
				return false;
			}*/
			// User login
	...
	if ( ! empty( $user['user_id'] ) && $user['user_id'] > 0 && !empty( $_POST["rpress_phone"] ) ) {
		update_user_meta( $user['user_id'], '_rpress_phone', $_POST["rpress_phone"] );
	}
