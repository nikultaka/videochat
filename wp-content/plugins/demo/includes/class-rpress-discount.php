...
class RPRESS_Discount {
	...
	public function is_started( $set_error = true ) {
		...
		if ( $this->start ) {
			...
			if ( $start_date < current_time( 'timestamp' ) ) {
				...
				rpress_set_error( 'rpress-discount-error', _x( 'Voucher code is not valid.', 'error shown when attempting to use a discount before its start date', 'restropress' ) );
			}
		} else {
	...
	public function is_valid( $user = '', $set_error = true ) {
		...
		if ( rpress_get_cart_contents() && $this->ID ) {
			...
			rpress_set_error( 'rpress-discount-error', _x( 'Voucher code is not valid.', 'error for when a discount is invalid based on its configuration' , 'restropress' ) );
		}
