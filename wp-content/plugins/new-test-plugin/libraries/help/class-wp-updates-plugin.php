...
// LicenseBox Load the helper file to increase protection you can include some core functions of your plugin like innit() functions etc. in this file before you obfuscate it.
require 'lb_helper.php';

//  Licensebox method Create a new LicenseBoxAPI helper class.
$lbapi = new LicenseBoxAPI();

//  Licensebox method Performs background license check, pass TRUE as 1st parameter to perform periodic verifications only.
$lb_verify_res = $lbapi->verify_license();

//  Licensebox method Performs update check, you can easily change the duration of update checks.
...
// Licensebox method If user has a valid license and new updates are available show the update notification in plugins page.
if ( ( $lb_update_res['status'] ) && ( $lb_verify_res['status'] ) ) {
	function licensebox_show_update_notice() {
		...
		$update_notification = esc_html( $lb_update_res['LB_UPDATE'] );
...
if ( ! class_exists( 'WP_Updates_Plugin_lb_Updater' ) ) {
	...
	class WP_Updates_Plugin_lb_Updater {
		...
		function __construct() {
			...
			$this->plugin_path     = 'new-test-plugin/new-test-plugin.php';
			$this->plugin_slug     = 'new-test-plugin';
			$this->plugin_url      = '';
