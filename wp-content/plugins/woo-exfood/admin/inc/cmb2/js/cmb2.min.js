		...
		if ( $repeatableGroups.length ) {
			$repeatableGroups.sortable({
				...
				stop: function (ev) {
					...
					if($group.closest( '.cmb2-id-exwo-options' ).length){
					...
					}else if($group.closest( '.cmb2-id-exwfood-adv-timedeli' ).length){
						$group.find( '.cmb-repeatable-grouping' ).each( function( i, row ){
							var $this_gr = $(this);
							$this_gr.find( 'input' ).each( function(){
								var $_name = $(this).attr('name');
								var res =  $_name.split("][");
								$_name = $_name.replace( res[0], 'exwfood_adv_timedeli['+(iterator-1) );
								$(this).attr('name',$_name)
							});
							$this_gr.find( 'select' ).each( function(){
								var $_name = $(this).attr('name');
