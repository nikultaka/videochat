<?php
function exwoofood_shortcode_grid( $atts ) {
	if(phpversion()>=7){
		$atts = (array)$atts;
	}
...
add_shortcode( 'ex_wf_grid', 'exwoofood_shortcode_grid' );
add_action( 'after_setup_theme', 'ex_reg_wfood_grid_vc' );
function ex_reg_wfood_grid_vc(){
    if(function_exists('vc_map')){
	vc_map( array(
	   "name" => esc_html__("Food Grid", "woocommerce-food"),
	   "base" => "ex_wf_grid",
	   "class" => "",
	   "icon" => "icon-grid",
	   "controls" => "full",
