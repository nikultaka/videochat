...
class WCP_VideoChat_Controller {
    ...
    public function add_stream_user_old() {     
        global $wpdb;
        $user_id = '';
        $table = $wpdb->prefix.'online_users';
        $marbletable = $wpdb->prefix.'marble_position';
        ...
        if(isset($_POST['user_id']) && $type == 'local') {          
            /*$userData = $wpdb->get_results("select * from ".$table." where user_id =".$user_id." and status = '1' and room_id =".$room_id);    */
   
            //if(!empty($userData)) {
                if($stream_id == '') {
                    $wpdb->query(" update ".$table." set status = '1',stream_id='".$drone_id."',gm_updated = NOW(),is_video_enable = '".$is_video_enable."' where user_id = ".$user_id." and room_id =".$room_id);        
                } else {
                    $wpdb->query(" update ".$table." set status = '1',stream_id='".$stream_id."',gm_updated = NOW(),is_video_enable = '".$is_video_enable."' where user_id = ".$user_id." and room_id =".$room_id);        
                }  
           ...
           ");   
            ...
            echo '<pre>'; 
            print_r($joinroomData); 
            exit;

            $user_position = '';  

            foreach($joinroomData as $join_key => $join_value) {
                if( $stream_id == $join_value->stream_id)  {
                    $user_position = $join_key;     
                }              
            }  
            self::check_active_status($table,$joinroomtable);
